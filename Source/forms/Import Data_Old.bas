...
Private Sub Button24_Click()
    ...
    If Response = 6 Then
        ...
        DoCmd.RunSQL "DELETE DISTINCTROW ImportData.* FROM ImportData"
        [I_Data].Requery
    End If
...
On Error GoTo Err_ImportData_Click
  ...
  Set ITRS = Db.OpenRecordset("ImportData", dbOpenDynaset)   ' Create dynaset.
  ...
  Else
    ...
    While Not ITRS.EOF And Continue
      ...
      ElseIf CompetitorEnrolled(ITRS!G_name, ITRS!S_name) Then
        ...
        ActualSex = DetermineSex(ITRS!Sex)
        Criteria = "[Gname]=""" & ITRS!G_name & """ AND [Surname]=""" & ITRS!S_name & """ AND [Age] =""" & ITRS!Age & """ AND [H_Code]=""" & ITRS!H_Code & """" & " AND [Sex]=""" & ActualSex & """"
...
Err_ImportData_Click:
    MsgBox "There text file(s) that you are attempting to import appear to have corrupt data.  Please check verify that they are correct. (" & Error$ & ")"
...
Private Sub Locate_Click()
    ...
    'Me!ctlCommonDialog.DialogTitle = "Locate Team Text File"
    'Me!ctlCommonDialog.Filter = "Team Text File (*.txt)|*.txt|All (*.*)|*.*"
    'Me!ctlCommonDialog.DefaultExt = "txt"

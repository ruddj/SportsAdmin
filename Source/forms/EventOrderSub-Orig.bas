...
Private Sub E_Num_BeforeUpdate(Cancel As Integer)
...
Private Sub E_Num_Click()

  If Me.Parent.SingleClickOption Then Call DetermineNewEventNum
  
End Sub

Private Sub E_Num_DblClick(Cancel As Integer)
  
  Call DetermineNewEventNum

End Sub

Private Sub E_Num_Enter()
...
Private Sub Etime_AfterUpdate()

  LastTime = Me!Etime
  
End Sub

Private Sub Etime_Enter()
On Error GoTo Etime_Enter_Exit
  
  If Not VarEmpty(Me!E_Num) Then
    If IsNull(Me!Etime) Then
      Me!Etime = LastTime
    End If
  End If
